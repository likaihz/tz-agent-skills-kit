...
9. Verify declared outcomes directly. For work-ledger, require CLI `>=0.11.0,<1.0.0`, protocol 1, Vault schema 5, commands `knowledge.list` and `knowledge.show`, and `features.knowledge_documents=true`. Stop and block all business writes until every gate passes; never use an older runnable CLI as a partial fallback.
