...
3. Compare product identity, version, protocol, and every required command and feature. For work-ledger, the base gate is CLI `>=0.11.0,<1.0.0`, protocol 1, commands `knowledge.list` and `knowledge.show`, and `features.knowledge_documents=true`.
...
9. Run the target's temporary smoke test. Work-ledger must initialize schema 5 and query created Knowledge through both required commands.
...
- CLI is `>=0.11.0,<1.0.0`, protocol 1, and reports `features.reportable_project_default=true` and `features.knowledge_documents=true`;
- capabilities contains commands `knowledge.list` and `knowledge.show` with that exact spelling;
- a temporary initialization produces Vault schema 5, and the intended Vault is schema 5 before any business write;
...
5. For schema 1–4, use the verified candidate to run `migrate plan --to 5`; inspect collisions, unmanaged links, child-project changes, and the `4->5-knowledge-documents` step.
6. Apply only the exact reviewed `plan_digest` after the candidate runtime passes its temporary smoke test; verify the single migration commit, schema 5, and `doctor` result.
