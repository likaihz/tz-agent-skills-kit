...
3. Compare product identity, version, protocol, and every required command and feature. For work-ledger, the base gate is CLI `>=0.11.0,<1.0.0`, protocol 1, commands `knowledge.list` and `knowledge.show`, `features.knowledge_documents=true`, `features.knowledge_kind_document=true`, and `features.knowledge_kind_memo=true`.
...
9. Run the target's temporary smoke test. Work-ledger must initialize schema 7 and query created `document` and `memo` Knowledge through both required commands.
...
- CLI is `>=0.11.0,<1.0.0`, protocol 1, and reports `features.reportable_project_default=true`, `features.knowledge_documents=true`, `features.knowledge_kind_document=true`, and `features.knowledge_kind_memo=true`;
- capabilities contains commands `knowledge.list` and `knowledge.show` with that exact spelling;
- a temporary initialization produces Vault schema 7, and the intended Vault is schema 7 before any business write;
...
5. For schema 1–6, use the verified candidate to run `migrate plan --to 7`; inspect collisions, unmanaged links, child-project changes, `4->5-knowledge-documents`, byte-preserving `5->6-document-knowledge-kind`, and byte-preserving `6->7-memo-knowledge-kind`.
6. Apply only the exact reviewed `plan_digest` after the candidate runtime passes its temporary smoke test; verify the single migration commit, schema 7, and `doctor` result.
